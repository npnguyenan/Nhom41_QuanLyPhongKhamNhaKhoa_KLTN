...
/**
 * Trả về class CSS Bootstrap tương ứng cho mỗi loại trạng thái lịch hẹn
 * 
...
/**
 * Trả về tên hiển thị cho mỗi trạng thái lịch hẹn
 * 
 * @param string $status Mã trạng thái
...
/**
 * Trả về biểu tượng Font Awesome tương ứng cho mỗi trạng thái
 * 
 * @param string $status Mã trạng thái
...
/**
 * Gửi email sử dụng thư viện PHPMailer
 *
 * @param string $to Địa chỉ email người nhận
 * @param string $subject Tiêu đề email
 * @param string $message Nội dung email (HTML)
 * @param array $attachments Đường dẫn đến các tệp đính kèm (tùy chọn)
 * @return bool Kết quả gửi email
 */
function send_email($to, $subject, $message, $attachments = []) {
    // Lấy cấu hình email từ cài đặt hệ thống
    $smtp_host = get_setting('smtp_host', '');
    $smtp_port = get_setting('smtp_port', '587');
    $smtp_username = get_setting('smtp_username', '');
    $smtp_password = get_setting('smtp_password', '');
    $smtp_secure = get_setting('smtp_secure', 'tls'); // tls hoặc ssl
    $from_email = get_setting('site_email', 'no-reply@yourdomain.com');
    $from_name = get_setting('site_name', 'Hệ thống đặt lịch khám');
    ...
    // Khởi tạo PHPMailer
    $mail = new PHPMailer(true); // true = bật chế độ báo lỗi ngoại lệ
    ...
    try {
        ...
        if (!empty($smtp_host) && !empty($smtp_username) && !empty($smtp_password)) {
            ...
            // Cài đặt phương thức bảo mật khi kết nối với SMTP (tls hoặc ssl)
            if ($smtp_secure == 'tls') {
            ...
            // Tùy chọn gỡ lỗi
            // $mail->SMTPDebug = SMTP::DEBUG_SERVER; // Bật chế độ gỡ lỗi
        }
        
        // Người gửi và người nhận
        $mail->setFrom($from_email, $from_name);
        
        // Hỗ trợ nhiều địa chỉ email người nhận ngăn cách bằng dấu phẩy
        ...
        // Ghi log lỗi
        $log_file = __DIR__ . '/../logs/email_error_' . date('Y-m') . '.log';
        $error_message = date('Y-m-d H:i:s') . " | ERROR | To: $to | Subject: $subject | " . $mail->ErrorInfo . "\n";
