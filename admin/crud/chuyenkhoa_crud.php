<?php
// Turn off notices and warnings to prevent JSON corruption
...
function getAllSpecialties($search = '', $page = 1, $items_per_page = 10)
{
    ...
    if (!empty($search)) {
        ...
        $whereClause .= " AND (ck.ten_chuyenkhoa LIKE '%$search%' OR ck.mota LIKE '%$search%')";
    }
...
function addSpecialty($data, $image)
{
    ...
    $tenChuyenkhoa = isset($data['tenChuyenkhoa']) ? $conn->real_escape_string($data['tenChuyenkhoa']) : '';
    $mota = isset($data['mota']) ? $conn->real_escape_string($data['mota']) : '';
    
    if (empty($tenChuyenkhoa)) {
        return ["success" => false, "message" => "Tên chuyên khoa không được để trống"];
    }
    ...
    $sql = "INSERT INTO chuyenkhoa (ten_chuyenkhoa, mota, hinh_anh, ngay_tao) 
            VALUES ('$tenChuyenkhoa', '$mota', '$hinhAnh', NOW())";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action'])) {
    ...
    switch ($action) {
        case 'add':
            echo json_encode(addSpecialty($_POST, $_FILES['hinhAnh'] ?? null));
            break;
        case 'update':
            echo json_encode(updateSpecialty($_POST['id'] ?? 0, $_POST, $_FILES['hinhAnh'] ?? null));
            break;
        case 'delete':
            echo json_encode(deleteSpecialty($_POST['id'] ?? 0));
            break;
        default:
