...
<body>
                                    ...
                                    <div class="col-lg-6">
                                        ...
                                        <table class="table table-hover detail-table">
                                            ...
                                            <tr>
                                                ...
                                                <td class="rounded-end" id="appointment-status-container">
                                                    <div class="d-flex align-items-center">
                                                        <span id="appointment-status-badge" 
                                                            class="badge rounded-pill px-3 py-2 bg-secondary">
                                                            <i class="fas fa-spinner fa-spin me-1"></i>
                                                            Đang tải...
                                                        </span>
                                                        <button id="refresh-status-btn" class="btn btn-sm ms-2" title="Làm mới trạng thái">
    ...
    <?php if ($medical_record): ?>
    <script>
        // Biến toàn cục lưu thông tin lịch hẹn
        const appointmentInfo = {
            ma_lichhen: '<?php echo htmlspecialchars($medical_record['ma_lichhen']); ?>',
            sdt: '<?php echo htmlspecialchars($medical_record['dien_thoai']); ?>'
        };

        // Lấy trạng thái lịch hẹn qua API
        function getAppointmentStatus() {
            // Hiển thị trạng thái đang tải
            document.getElementById('appointment-status-badge').innerHTML = '<i class="fas fa-spinner fa-spin me-1"></i> Đang tải...';
            document.getElementById('appointment-status-badge').className = 'badge rounded-pill px-3 py-2 bg-secondary';
            
            // Thực hiện AJAX request
            fetch(`get_appointment_status.php?ma_lichhen=${appointmentInfo.ma_lichhen}&sdt=${appointmentInfo.sdt}`)
                .then(response => {
                    if (!response.ok) {
                ...
                .catch(error => {
                    ...
                    document.getElementById('appointment-status-badge').innerHTML = '<i class="fas fa-exclamation-triangle me-1"></i> Lỗi kết nối';
                    document.getElementById('appointment-status-badge').className = 'badge rounded-pill px-3 py-2 bg-danger';
                });
        }
        ...
        function updateStatusUI(data) {
            if (data && data.status) {
                ...
                // Cập nhật nội dung và lớp CSS cho badge
                statusBadge.innerHTML = `<i class="${data.status_icon} me-1"></i> ${data.status_name}`;
                statusBadge.className = `badge rounded-pill px-3 py-2 ${data.status_badge}`;
        ...
        // Xử lý nút làm mới trạng thái
        document.getElementById('refresh-status-btn').addEventListener('click', function(e) {
            e.preventDefault();
            getAppointmentStatus();
            
            // Hiệu ứng xoay cho nút refresh
        ...
        // Tự động lấy trạng thái khi tải trang
        document.addEventListener('DOMContentLoaded', function() {
            getAppointmentStatus();
            
            // Tự động cập nhật trạng thái mỗi 30 giây
            setInterval(getAppointmentStatus, 30000);
        });
    </script>
    <?php endif; ?>
</body>
